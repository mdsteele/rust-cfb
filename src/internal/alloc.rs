...
impl<F> Allocator<F> {
    pub fn new(sectors: Sectors<F>, difat: Vec<u32>, fat: Vec<u32>)
...
impl<F: Write + Seek> Allocator<F> {
    ...
    pub fn extend_chain(&mut self, start_sector: u32, init: SectorInit)
                        -> io::Result<u32> {
        debug_assert_ne!(start_sector, consts::END_OF_CHAIN);
